...
public class ISAReader extends DefaultIteratingChemObjectReader implements
		IRawReader<IStructureRecord>, ICiteable {
	...
	public boolean hasNext() {
		...
		if (assayResultIterator == null) {
			if (studyIterator.hasNext()) {
				...
				assayResultIterator = ((Study) object).getAssayResults()
						.iterator();
	...
	protected SubstanceRecord parseAssayResult(AssayResult result) {
		...
		try {
			...
			a_papp.setReferenceYear(Integer.toString(calendar
					.get(Calendar.YEAR)));
		} catch (Exception x) {
		}
		...
		for (FactorValue pv : factorvalues) {
			...
			if ("compound".equals(f.getValue())	|| "limiting nutrient".equals(f.getValue())) {
				...
				if (term != null) {
					...
					record.setCompanyUUID("ISTB-"
							+ UUID.nameUUIDFromBytes(term.getAcc().getBytes()));
					record.setOwnerName(term.getSource().getUrl());
					record.setOwnerUUID("ISTB-"
							+ UUID.nameUUIDFromBytes(term.getSource().getName()
									.getBytes()));
					record.setFormat("ISATAB");
					List<ExternalIdentifier> ids = new ArrayList<ExternalIdentifier>();
					record.setExternalids(ids);
					record.setSubstancetype("compound");
					ids.add(new ExternalIdentifier(
							term.getSource().getUrl() == null ? "ISA-TAB"
									: term.getSource().getUrl(), term.getAcc()));
				} else {
					...
					record.setOwnerUUID("ISTB-"
							+ UUID.nameUUIDFromBytes("ISA-TAB".getBytes()));
					record.setFormat("ISATAB");
					record.setSubstancetype("compound");
				}
			} else {
	...
	protected void processCharacteristicValues(
			Collection<CharacteristicValue> characteristicValues, Params params) {
	...
	protected void processParamValues(Collection<ParameterValue> paramValues,
			Params params) {
				...
				if (papps != null)
					for (Object p : papps)
						if (p instanceof ProtocolApplication) {
							...
							for (uk.ac.ebi.bioinvindex.model.Annotation annotation : pa
									.getAnnotations()) {
								if ("sampleFileId".equals(annotation.getType()
										.getValue())) {
									assayField = false;
									break;
								}
