...
public class PubChemAIDMetadata {
	...
	public String getAIDSource_id() {
		return metadata.get("assay").get("descr").get("aid_source").get("db").get("source_id").get("str").getTextValue();
	}
	...
	public String getAIDSource_date() {
		try {
			...
			return String.format("%s-%s-%s", date.get("year").getTextValue(),date.get("month").getTextValue(),date.get("day").getTextValue());
		} catch (Exception x) {
			return null;
		}
	}
