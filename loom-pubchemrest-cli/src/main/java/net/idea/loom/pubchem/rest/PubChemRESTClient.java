...
public class PubChemRESTClient<T extends IIdentifiableResource<IIdentifier>>
	...
	protected List<T> processPayloadWithCallback(InputStream in,
			String mediaType, ICallback<List<T>> callback)
			throws RestException, IOException {
		if (callback == null) {
	...
	public List<T> getSubstanceSynonyms(String pubchem_sid)
		...
		return get(new Identifier(String.format("%s/%s/sid/%s/%s/%s", pubchem_rest,
				command_input.substance.name(), pubchem_sid,
				command_operation.synonyms.name(), command_output.json.name())),
				mime_json, callback, null);
	}

	@Override
	protected List<T> get(IIdentifier url, String mediaType, String... params)
			throws RestException, IOException {
		return get(url, mediaType, null, params);
	}

	protected List<T> get(IIdentifier url, String mediaType,
			ICallback<List<T>> callback, String... params)
			throws RestException, IOException {
		String address;
		try {
			address = prepareParams(url, params);
 ...
 */
class SubstanceSynonymsCallback<T extends IIdentifiableResource<String>>
		implements ICallback<List<T>> {

	@Override
	public List<T> processJSON(JsonNode node) {
		List<T> list = new ArrayList<T>();
		try {
			ArrayNode info = (ArrayNode) node.get("InformationList").get(
					"Information");
			for (int i = 0; i < info.size(); i++) {
				...
				for (int j = 0; j < synonyms.size(); j++) {
					...
					T id = (T) new PubChemIdentifier(type, value);
					list.add(id);
				}
			}
			return list;
