...
public class OPSPharmacologyClientTest extends
		AbstractOPSClientTest<AssayResult, OPSPharmacologyClient> {
	...
	public void testCompoundPharmacology() throws Exception {
		Compound cmp = new Compound(new Identifier(
				"http://rdf.chemspider.com/236"));
		OPSPharmacologyClient cli = getOPSClient();
		List<AssayResult> list = cli.getCompoundPharmacology(new URL(
				TEST_SERVER), cmp, 0, 5);
	...
	public void testCompoundPharmacologyCount() throws Exception {
		Compound cmp = new Compound(new Identifier(
				"http://rdf.chemspider.com/236"));
		OPSPharmacologyClient cli = getOPSClient();
		Integer count = cli.getCompoundPharmacologyCount(new URL(TEST_SERVER),
				cmp);
	...
	public void testTargetPharmacologyCount() throws Exception {
		Target target = new Target(new Identifier(
				"http://www.uniprot.org/uniprot/P19099"));
		OPSPharmacologyClient cli = getOPSClient();
		Integer count = cli.getTargetPharmacologyCount(new URL(TEST_SERVER),
				target);
		Assert.assertTrue(count.intValue() > 0);
	}
	...
	public void testTargetPharmacology() throws Exception {
		...
		OPSPharmacologyClient cli = getOPSClient();
		int pagesize = 100;
		List<AssayResult> list = cli.getTargetPharmacology(
				new URL(TEST_SERVER), target, 0, pagesize);
		Assert.assertTrue(list.size() > 0);
