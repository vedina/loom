...
public class OPSCompoundClientTest extends AbstractOPSClientTest<Compound, OPSCompoundClient> {
	...
	OPSCompoundClient cli = getOPSClient();
	List<Compound> list = cli.searchStructuresByInchi(new URL(TEST_SERVER),
    ...
    /*
     * @Test public void testReadIdentifiers() throws Exception {
     * OPSCompoundClient opsClient = getOPSClient(); //get the first record
     * List<Compound> substances = opsClient.getIdentifiersAndLinks( new
     * URL(String.format("%s", TEST_SERVER)),//bosentan new
     * URL(String.format("%s%s/1", TEST_SERVER,Resources.compound)) );
     * Assert.assertNotNull(substances); for (Compound s : substances) {
