...
public class OPSPharmacologyClient extends AbstractOPSClient<AssayResult> {
    ...
    public List<AssayResult> getCompoundPharmacology(URL queryService, Compound cmp, int page, int pagesize)
	...
	return get(ref, mime_json, params.uri.name(), cmp.getResourceIdentifier().toExternalForm(),
		pagination._page.name(), Integer.toString(page), pagination._pageSize.name(),
		Integer.toString(pagesize), "_format", _format.json.name());
    }

    public List<AssayResult> getTargetPharmacology(URL queryService, Target target, int page, int pagesize)
	    throws RestException, IOException {
	URL ref = new URL(String.format("%s/target%s/pages", queryService, resource));
	return get(ref, mime_json, params.uri.name(), target.getResourceIdentifier().toExternalForm(),
		pagination._page.name(), Integer.toString(page), pagination._pageSize.name(),
		Integer.toString(pagesize), "_orderBy", "?compound_chembl", "_format", _format.json.name());
    }
	...
	return getCount("compoundPharmacologyTotalResults", ref, mime_json, params.uri.name(), cmp
		.getResourceIdentifier().toExternalForm(), "_format", _format.json.name());
    }
	...
	return getCount("targetPharmacologyTotalResults", ref, mime_json, params.uri.name(), target
		.getResourceIdentifier().toExternalForm(), "_format", _format.json.name());
    }

    @Override
    protected List<AssayResult> processPayload(InputStream in, String mediaType) throws RestException, IOException {
	...
	if (uri != null && uri.getTextValue() != null) {
		...
		if (matchedURI != null && (matchedURI.getTextValue() != null)) {
		    ...
		    if (label != null)
			target.setInDataset(datasets.lookup(label.getTextValue()));
		}
	    }
