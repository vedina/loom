...
public class OPSCompoundClient extends AbstractOPSClient<Compound> {
	...
	return get(ref, mime_json, searchOptions.Molecule.key(), term, "_format", _format.json.name());
    }
     ...
     * @throws IOException
     */
    public List<Compound> searchSimilarStructures(URL queryService, String term, double threshold, int limit)
	    throws RestException, IOException {
	URL ref = new URL(String.format("%s%s/%s", queryService, resource, QueryType.similarity.name()));
	return get(ref, mime_json, searchOptions.Molecule.key(), term, searchOptions.Threshold.key(),
		Double.toString(threshold), "_format", _format.json.name(), resultOptions.Limit.name(),
		Integer.toString(limit));
    }

    public List<Compound> searchSubstructures(URL queryService, String term, int limit) throws RestException,
	    IOException {
	URL ref = new URL(String.format("%s%s/%s", queryService, resource, QueryType.substructure.name()));
	return get(ref, mime_json, searchOptions.Molecule.key(), term, resultOptions.Limit.name(),
		Integer.toString(limit), "_format", _format.json.name());
    }
    ...
    @Override
    protected List<Compound> processPayload(InputStream in, String mediaType) throws RestException, IOException {
	...
	if (mime_json.equals(mediaType)) {
		...
		if (primaryTopic.get("Molecule") != null)
		    compound.setSMILES(primaryTopic.get("Molecule").getTextValue());
		if (primaryTopic.get("smiles") != null)
		    compound.setSMILES(primaryTopic.get("smiles").getTextValue());
		if (primaryTopic.get("inchi") != null)
		    compound.setInChI(primaryTopic.get("inchi").getTextValue());
		if (primaryTopic.get("inchi_key") != null)
		    compound.setInChIKey(primaryTopic.get("inchi_key").getTextValue());
	    }
	    return list;
    ...
    @Override
    protected Compound parseItem(ObjectMapper m, JsonNode result, JsonNode item) throws MalformedURLException {
	return null;
    }
}
