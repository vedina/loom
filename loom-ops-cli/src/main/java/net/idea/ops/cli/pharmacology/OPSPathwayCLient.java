...
public class OPSPathwayClient extends AbstractOPSClient<Pathway> {
	...
	public List<Pathway> getPathwaysByCompound(URL queryService,Compound cmp,int page,int pagesize) throws RestException,IOException {
		...
		return get(ref,mime_json,
				...
				pagination._pageSize.name(),(pagesize<=0)?"all":Integer.toString(pagesize),
				"_format",_format.json.name());
	}		
	@Override
	protected Pathway parseItem(ObjectMapper m, JsonNode result, JsonNode item)
		...
		if (node!=null) pathway.setTitle(node.getTextValue());
		else {
			node = item.get("title");
			if (node!=null) pathway.setTitle(node.getTextValue());
		}
