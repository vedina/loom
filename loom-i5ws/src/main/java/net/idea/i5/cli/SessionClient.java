...
public class SessionClient extends I5AbstractClient {
    ...
    @Override
    protected List<IIdentifiableResource<String>> get(String url, String mediaType, String... params)
	...
	httpPOST.addHeader(
		"Content-type",
		String.format(
			"application/soap+xml;charset=UTF-8;action=\"http://echa.europa.eu/iuclid5/i5webservice/service/SessionEngine/%s\"",
			command));
	...
	loginResponse, session, emptyResponse
    }
	...
	try {
		...
		case XMLStreamConstants.START_ELEMENT: {
			...
			case session: {
			    session = reader.getAttributeValue(
				    "http://echa.europa.eu/schemas/iuclid5/i5webservice/types/", "id");
			    break;
			}
			}
		    } catch (Exception x) {
