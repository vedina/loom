...
public class QueryToolClientTest extends ClientTest {
	...
	try {
		...
		String[] identifier = { "IUC4-b128a38e-a298-3e99-b74e-2cee846cffc3/0",
			"IUC4-79206c66-7a3a-3d66-a670-e1a693dc81cf/0" };
		List<IIdentifiableResource<String>> content = cli.executeQuery(PredefinedQuery.query_by_it_identifier,
			"IUCLID4", "DSN 20");
	...
	try {
		...
		try {
		    List<IIdentifiableResource<String>> content = cli.executeQuery(
			    PredefinedQuery.query_by_it_identifier, "CompTox", "Ambit Transfer");
			...
			boolean found = false;
			for (String id : identifier)
			    if (id.equals(content.get(j).getResourceIdentifier())) {
				found = true;
				break;
			    }
			if (!found)
