...
public class CSV12Header extends StringArrayHeader<I5_ROOT_OBJECTS> {
	public enum _lines {
		endpointcategory {
			...
			public String getDescription() {
				...
				b.append(". One of :\n\n");
				b.append("<table>\n");
				for (I5_ROOT_OBJECTS root : I5_ROOT_OBJECTS.values())
					if (root.isScientificPart()) {
						b.append("<tr>");
						b.append("<td>");
						b.append(root.name());
						b.append("</td>");
		...
		protocol, guideline, type_of_study, type_of_method, data_gathering_instruments, endpoint, condition1 {
			@Override
	...
	public void assign(SubstanceRecord record, Object value) {
		...
		if ("Change".equals(lines[_lines.condition2.ordinal()]))
			return;
			...
			if ("SMILES".equals(lines[_lines.endpoint.ordinal()])) {
				...
				;
				try {
					structure.setContent(value.toString());
			...
			if ("Element".equals(lines[_lines.endpoint.ordinal()])) {
				...
				Protocol protocol = I5_ROOT_OBJECTS.SURFACE_CHEMISTRY
						.getProtocol(lines[_lines.data_gathering_instruments.ordinal()]);
				protocol.addGuideline(lines[_lines.data_gathering_instruments.ordinal()]);

				ReliabilityParams rel = new ReliabilityParams();
				if (lines[_lines.type_of_method.ordinal()].contains("calculation")
						|| lines[_lines.type_of_method.ordinal()].contains("simulation")) {
					rel.setStudyResultType("estimated by calculation");
				} else {
					rel.setStudyResultType("experimental result");
				}
				ProtocolApplication<Protocol, IParams, String, IParams, String> experiment = getExperiment(
						I5_ROOT_OBJECTS.SURFACE_CHEMISTRY, record, protocol, rel);
				...
				// effect.getConditions().put("ELEMENT_OR_GROUP", new
				// Value(value));
				...
				;
				try {
					core.setContent(value.toString());
					core.setFormat("INC");
			...
			if (num != null) {
				...
				if (!"".equals(lines[_lines.data_gathering_instruments.ordinal()]))
					experiment.getParameters().put(I5CONSTANTS.pDATA_GATHERING_INSTRUMENTS,
				...
				((Params) effect.getTextValue()).put(lines[_lines.result.ordinal()], new Value(num, null));
			...
			if (("".equals(line) || "mean".equals(line) || "sd".equals(line) || "std".equals(line)
					|| "sem".equals(line) || "interpretation".equals(line) || "criteria".equals(line)
					|| "text".equals(line) || "condition".equals(line))) {
				I5_ROOT_OBJECTS category = I5_ROOT_OBJECTS.UNKNOWN_TOXICITY;
				try {
					category = category.valueOf(lines[_lines.endpointcategory.ordinal()]);
				} catch (Exception x) {
				...
				ProtocolApplication<Protocol, IParams, String, IParams, String> experiment = getExperiment(category,
						record, protocol, rel);
				...
				/*
				 * doens't work if ("condition".equals(line)) { String name =
				 * lines[_lines.endpoint.ordinal()]; try { Value cValue = new
				 * Value(Double.parseDouble(value.toString()));
				 * cValue.setUnits(lines
				 * [_lines.units.ordinal()]==null?null:lines
				 * [_lines.units.ordinal()].trim());
				 * effect.getConditions().put(name, cValue); } catch (Exception
				 * x) { effect.getConditions().put(name, value.toString()); }
				 * experiment.addEffect(effect); }
				 */
				if ("mean".equals(line) || "".equals(line) || "interpretation".equals(line) || "criteria".equals(line)
						|| "text".equals(line)) {
					String endpoint = lines[_lines.endpoint.ordinal()];
					effect.setEndpoint(endpoint);

					for (_lines field : _lines.values())
	...
	protected EffectRecord<String, IParams, String> getEffectRecord(I5_ROOT_OBJECTS category,
			ProtocolApplication<Protocol, IParams, String, IParams, String> experiment) {
		String sampleUUID = prefix
				+ UUID.nameUUIDFromBytes((lines[_lines.endpointcategory.ordinal()] + lines[_lines.protocol.ordinal()]
						...
						+ lines[_lines.endpoint.ordinal()] + lines[_lines.condition1.ordinal()]
						+ lines[_lines.condition2.ordinal()] + lines[_lines.condition3.ordinal()]).getBytes());
	...
	protected ProtocolApplication<Protocol, IParams, String, IParams, String> getExperiment(I5_ROOT_OBJECTS category,
		...
		String experimentUUID = prefix
				+ UUID.nameUUIDFromBytes((record.getSubstanceUUID() + category.name()
						+ lines[_lines.protocol.ordinal()] + lines[_lines.guideline.ordinal()]
						+ lines[_lines.type_of_method.ordinal()] + lines[_lines.type_of_study.ordinal()] + lines[_lines.data_gathering_instruments
						.ordinal()]
	...
	protected String getConditionValue(String cell) {
		return cell;
		/*
		 * String medium = null; if ((cell!=null) && cell.indexOf("serum")>0) {
		 * medium = "Human serum"; } return medium;
		 */
	}
